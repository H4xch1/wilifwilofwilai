...
export default function MuridPanel({ activePanel }) {
  ...
  const checkSudahAbsen = async () => {
    try {
      ...
      // Bandingin tanggal pake split('T')[0]
      const sudah = (Array.isArray(res.data) ? res.data : []).some(
  ...
  const openDetail = (absen) => setSelectedAbsen(absen);
  const closeDetail = () => setSelectedAbsen(null);
